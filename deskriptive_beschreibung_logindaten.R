...
# Eindeutige Liste nur f??r Kunden
...
kennzahlenBerechner(df_light$anzahl_login_kunde, "Anzahl Logins pro Kunde")

par(mfrow = (c(1,2)))
boxplot(df_light$anzahl_login_kunde,
        main = "Anzahl Logins pro Kunde (mit Ausreisser)",
        ylab = "Anzahl Logins",
        col = "lightblue")

q <- quantile(df_light$anzahl_login_kunde, probs = c(0.25, 0.75), na.rm = TRUE)
iqr <- q[2] - q[1]
untere_grenze <- q[1] - 1.5 * iqr
obere_grenze <- q[2]+1.5*iqr


subset_wO_Outliers <- df_light[df_light$anzahl_login_kunde <=  obere_grenze & df_light$anzahl_login_kunde >=  untere_grenze,]
...
# Anzahl Login pro User
View(df_kunde_distinct)

kennzahlenBerechner(df_kunde_distinct$anzahl_user, "Anzahl User pro Kunde")
mean_anz_user <- round(mean(df_light$anzahl_user, na.rm = TRUE),2)

ggplot(data=df_light, aes(x= anzahl_user))+
  geom_histogram(binwidth = 1, fill="steelblue", color= "black")+
  geom_vline(aes(xintercept = mean_anz_user), color="red",linetype="dashed", size=1)+
  annotate("text", x = mean_anz_user+15, y=7.5,label=paste0("mean: ",mean_anz_user),color="red",vjust=1.5, angle=0)+
  labs(title= "Verteilung der Anzahl User pro Kunde", x= "Anzahl User", y="Anzahl Kunden")+
  scale_x_continuous(breaks = seq(0, max(df_light$anzahl_user, na.rm = TRUE), by = 5))
